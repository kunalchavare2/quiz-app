...
import { LoadingWrapper, UsersWrapper } from "./Users.styled";
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        let userData = response.data.filter(
          (data) => data.tag.toLowerCase() === "user"
        );
        if (response.status === 200) {
          setTimeout(() => {
            ...
            setFilterData(() => JSON.parse(JSON.stringify(response.data)));
          }, 1000);
        }
      })
      .catch((error) => {
